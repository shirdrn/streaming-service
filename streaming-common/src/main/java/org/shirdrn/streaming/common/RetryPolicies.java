...
public class RetryPolicies {
  ...
  /**
   * <p>
   * A retry policy for SocketException
   * Set a default policy with some explicit handlers for specific exceptions.
   * </p>
   */
  public static final RetryPolicy retryBySocketException(
      RetryPolicy defaultPolicy,
      Map<Class<? extends Exception>, RetryPolicy> exceptionToPolicyMap) {
    return new SocketExceptionDependentRetry(defaultPolicy, exceptionToPolicyMap);
  }
  ...
  static class SocketExceptionDependentRetry implements RetryPolicy {

    RetryPolicy defaultPolicy;
    Map<String, RetryPolicy> exceptionNameToPolicyMap;
    
    public SocketExceptionDependentRetry(RetryPolicy defaultPolicy,
    ...
    public boolean shouldRetry(Exception e, int retries) throws Exception {
      ...
      if (e instanceof SocketException) {
        policy = exceptionNameToPolicyMap.get(
            ((SocketException) e).getClass().getName());
      }
      if (policy == null) {
        policy = defaultPolicy;
      }
